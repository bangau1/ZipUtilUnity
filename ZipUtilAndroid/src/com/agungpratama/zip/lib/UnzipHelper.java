...
public class UnzipHelper {
	...
	public static boolean unzip(String path, String destination, UnzipHelperArgs arguments){
	...
	public static boolean unzip(File zipFile, File destinationFolder, UnzipHelperArgs arguments){
		boolean result = true;
		CountingInputStream is;
		ZipInputStream zis;
		
		UnzipHelperArgs arg = arguments;
		if(arg == null) arg = getDefaultArgs();
		...
		try{
			...
			while((ze = zis.getNextEntry())!= null){
				...
				try{
					...
					if(ze.isDirectory()){
						...
						if(parentF!=null && !parentF.exists())
							parentF.mkdirs();
						FileOutputStream fos = new FileOutputStream(destinationFolderName + fileName);
						while((arg.bytesRead = zis.read(arg.buffer))!=-1){
							fos.write(arg.buffer, 0, arg.bytesRead);
							if(arg.unzipListener!=null)
								arg.unzipListener.unzipProgressCallback((0.0f+is.getTotalBytesRead())/totalFileSize);
						}
						fos.close();
					}
					zis.closeEntry();
				...
				if(arg.unzipListener!=null){
					arg.unzipListener.unzipProgressCallback((0.0f+is.getTotalBytesRead())/totalFileSize);
					arg.unzipListener.unzipFileDecompressedCallback(fileName, unzipIndividualStatus);
				}
			}
