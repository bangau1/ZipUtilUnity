...
public class UnzipUnityInterface {
	...
	public static void unzipProgressCallback(final float progress) {
		...
		UnityPlayer.currentActivity.runOnUiThread(new Runnable() {
			...
			@Override
			public void run() {
				UnityPlayer.UnitySendMessage(UNITY_OBJ_NAME,
						"OnUnzipProgressCallback", progress + "");
			}
		});
	...
	public static void unzipFinishedCallback(final int status) {
		...
		UnityPlayer.currentActivity.runOnUiThread(new Runnable() {
			...
			public void run() {
				UnityPlayer.UnitySendMessage(UNITY_OBJ_NAME,
						"OnUnzipFinishedCallback", status + "");
			}
		});
	}

	public static void unzipFileDecompressedCallback(final String name,
			final int status) {
		Log.d(TAG, "unzipFileDecompressedCallback:" + name + " status:"
				+ status);
		UnityPlayer.currentActivity.runOnUiThread(new Runnable() {
			...
			public void run() {
				UnityPlayer.UnitySendMessage(UNITY_OBJ_NAME,
						"OnUnzipFileDecompressedCallback", name + ";;;"
								+ status);
			}
		});
	}
